...
void cacheInit()
{
    ...
    s.access_time = 0;
    CacheConfig setting1(32768, 8, 64, 0, 1);
    CacheConfig setting2(262144, 8, 512, 0, 1);
    CacheConfig setting3(8388608, 8, 16384, 0, 1);
    l1.SetConfig(setting1);
    l1_inst.SetConfig(setting1);
    l2.SetConfig(setting2);
    l3.SetConfig(setting3);
    memory.SetStats(s);
    l1.SetStats(s);
    l1_inst.SetStats(s);
    l2.SetStats(s);
...
void cacheBack()
{
    l1.pushBack(pushHit, pushCycle);
    l2.pushBack(pushHit, pushCycle);
    l3.pushBack(pushHit, pushCycle);
}
void checkReg()
{
    for (int i = 0; i < 8; i++)
...
void pipeline()
{
    ...
    cout << "Total cycles: " << realCycle + 1 << endl;
    fprintf(res, "Total cycles: %d\n", realCycle + 1);
    double cpi = (double)realCycle / inst_num;
    cout << "CPI: " << cpi << endl;
    fprintf(res, "CPI: %lf\n", cpi);
    cout << "Bubbles caused by wrong prediction: " << jmpBubble << endl;
    fprintf(res, "Bubbles caused by wrong prediction: %d\n", jmpBubble);
    cout << "Stall caused by data hazard: " << stall << endl;
    fprintf(res, "Stall caused by data hazard: %d\n", stall);
    fprintf(res, "\n");
    fclose(res);
}
...
void IF()
{
	...
	l1_inst.HandleRequest(PC, 4, 1, content, hit, realCycle);
	realCycle -= 1;
...
//高位乘：10,比大小：11,取余：12, 算术右移：13
    ...
    if (OP == OP_R || OP == OP_RW || OP == OP_BEQ || OP == OP_SW)
    {
        ...
        bool forward1 = ((rs1 == ID_EX.Rd && ID_EX.Ctrl_WB_MemtoReg == 0 && ID_EX.Ctrl_WB_RegWrite == 1) || (rs1 == EX_MEM.RegDst && rs1 != ID_EX.Rd));
        bool forward2 = ((rs2 == ID_EX.Rd && ID_EX.Ctrl_WB_MemtoReg == 0 && ID_EX.Ctrl_WB_RegWrite == 1) || (rs2 == EX_MEM.RegDst && rs2 != ID_EX.Rd));


        if ((flag1 && (!forward1)) || (flag2 && (!forward2)))
        {
            //printf("flag1:%d flag2:%d forward1:%d forward2:%d\n", flag1, flag2, forward1, forward2);
...
void EX()
{
	...
	//转发
    if ((rs1 == EX_MEM.RegDst || rs1 == MEM_WB.RegDst) && (rs1 != 0))
    {
        cout << "forward rs1!" << endl;
        if ((rs1 == EX_MEM.RegDst && EX_MEM.Ctrl_WB_MemtoReg == 0 && EX_MEM.Ctrl_WB_RegWrite == 1) || (rs1 == MEM_WB.RegDst && MEM_WB.Ctrl_WB_RegWrite == 1))
        {
            if (rs1 == MEM_WB.RegDst && MEM_WB.Ctrl_WB_RegWrite == 1)
            {
                if (MEM_WB.Ctrl_WB_MemtoReg == 1)
                {
                    if (branch == 2)
                        jalr = (long long)MEM_WB.Mem_read;
                    else
                        in1 = (long long)MEM_WB.Mem_read;
                }
                else
                {
                    if (branch == 2)
                        jalr = (long long)MEM_WB.ALU_out;
                    else
                        in1 = (long long)MEM_WB.ALU_out;
                }
            }
            if (rs1 == EX_MEM.RegDst && EX_MEM.Ctrl_WB_MemtoReg == 0 && EX_MEM.Ctrl_WB_RegWrite == 1)
            {
                if (branch == 2)
                    jalr = (long long)EX_MEM.ALU_out;
                else
                    in1 = (long long)EX_MEM.ALU_out;
            }
        }
    }
    if ((rs2 == EX_MEM.RegDst || rs2 == MEM_WB.RegDst) && (rs2 != 0) && (ALUSrc == 0))
    {
        cout << "forward rs2!" << endl;
        if ((rs2 == EX_MEM.RegDst && EX_MEM.Ctrl_WB_MemtoReg == 0 && EX_MEM.Ctrl_WB_RegWrite == 1) || (rs2 == MEM_WB.RegDst && MEM_WB.Ctrl_WB_RegWrite == 1))
        {
            if (rs2 == MEM_WB.RegDst && MEM_WB.Ctrl_WB_RegWrite == 1)
            {
                if (MEM_WB.Ctrl_WB_MemtoReg == 1)
                    in2 = (long long)MEM_WB.Mem_read;
                else
                    in2 = (long long)MEM_WB.ALU_out;
            }
            if (rs2 == EX_MEM.RegDst && EX_MEM.Ctrl_WB_MemtoReg == 0 && EX_MEM.Ctrl_WB_RegWrite == 1)
                in2 = (long long)EX_MEM.ALU_out;
        }
    }
...
void MEM()
{
    ...
    if (read != 0)
    {
        ...
        {
        case 1:
            l1.HandleRequest(ALUout, 1, 1, content, hit, realCycle);
            temp = *(char *)content;
            break;
        case 2:
            l1.HandleRequest(ALUout, 2, 1, content, hit, realCycle);
            temp = *(short *)content;
            break;
        case 3:
            l1.HandleRequest(ALUout, 4, 1, content, hit, realCycle);
            temp = *(int *)content;
            break;
        case 4:
            l1.HandleRequest(ALUout, 8, 1, content, hit, realCycle);
            temp = *(long long *)content;
            break;
        default:;
        }
        mem_read = temp;
    ...
    if (write != 0)
    {
        ...
        {
        case 1:
            *(char *)content = (unsigned char)reg[rs2];
            l1.HandleRequest(ALUout, 1, 0, content, hit, realCycle);
            /*temp = (unsigned char)reg[rs2];
            *(unsigned char *)(memory + ALUout) = temp;*/
            break;
        case 2:
            *(short *)content = (unsigned short)reg[rs2];
            l1.HandleRequest(ALUout, 2, 0, content, hit, realCycle);
            /*temp = (unsigned short)reg[rs2];
            *(unsigned short *)(memory + ALUout) = temp;*/
            break;
        case 3:
            *(int *)content = (unsigned int)reg[rs2];
            l1.HandleRequest(ALUout, 4, 0, content, hit, realCycle);
            /*temp = (unsigned int)reg[rs2];
            *(unsigned int *)(memory + ALUout) = temp;*/
            break;
        case 4:
            *(long long *)content = (unsigned long long)reg[rs2];
            l1.HandleRequest(ALUout, 8, 0, content, hit, realCycle);
